...
public class CloudImageView extends View {
	...
	protected void onLayout(boolean paramBoolean, int left, int top, int right,
			int buttom) {
		if (paramBoolean) {
			width = right - left;
			fullWidth = 5 * width >> 2;
			drawable.setBounds(0, 0, fullWidth, buttom);
			this.getLayoutParams().height = drawable.getBitmap().getHeight()
					* right / drawable.getBitmap().getWidth();
		}
	}
